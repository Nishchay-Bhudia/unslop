...
F = "README.md"
GH = "nishchay-bhudia/un-ai-ify"

do(F, textwrap.dedent(f"""\
<div align="center">
...
"""), "docs(readme): add title, badges, and banner")
...
human."
"""), "docs(readme): add problem statement")
...
"""), "docs(readme): add differentiation section")
...
```bash
git clone https://github.com/{GH}.git
...
"""), "docs(readme): add how-it-works pipeline table")
...
"""), "docs(readme): add platform rules table")
...
[`examples/`](./examples).
"""), "docs(readme): add TikTok example teaser")
...
FAQ_ITEMS = [
    ("Will this help me beat AI detectors?",
     "No, and it's not trying to. Detector behavior changes constantly and has "
     "high false-positive rates on non-native English and technical writing. "
     "This skill optimizes for a human reader's attention, not a classifier."),
    ("Does it make up stats or personal stories to sound convincing?",
     "No. The [Guardrails section of SKILL.md](./SKILL.md#guardrails) explicitly "
     "forbids fabricating claims, credentials, or experiences. If your draft "
     "needs a number to land, the skill asks you for the real one."),
    ("Can I use it for multiple platforms from one draft?",
     "Yes — ask for a multi-platform pass (see Advanced Options in "
     "[`SKILL.md`](./SKILL.md#advanced-options)) and it adapts the same core "
     "message to each platform's actual pacing rules, not just a reformat."),
    ("Is this free?",
     "Yes, MIT-licensed, no paywalls, no usage limits, forever."),
    ("How do I add a new hook formula or platform rule?",
     "See [`CONTRIBUTING.md`](./CONTRIBUTING.md) — PRs adding real, "
     "non-fabricated examples are welcome."),
]
do(F, "\n## FAQ\n", "docs(readme): add FAQ section header")
for i, (q, a) in enumerate(FAQ_ITEMS, start=1):
    do(F, f"\n**{q}**\n{a}\n", f"docs(readme): add FAQ item {i}")
...
"""), "docs(readme): add roadmap, contributing, license, and footer")

print("README.md build done")
